...
def calculate_predicted_voltage_plus(theta_ca, phi_ca, theta_an, phi_an, psi, temperature, flow, current,
    ...
    ep = electrolyzer_parameters
    if ep.dim() == 1:
        ep = ep.unsqueeze(0)   # (8,) -> (1,8)
    elif ep.dim() != 2:
        raise ValueError(f"electrolyzer_parameters must be (8,) or (B,8), got {ep.shape}")
    ...
    sigma_ca1 = ep[:, 5:6]
    sigma_ca2 = ep[:, 6:7]
    area_cm2  = ep[:, 7:8]   # 或者你原来叫 Vdiff_base，但建议改名为 area_cm2
    i = current * area_cm2 # 4是电解槽膜面积
...
def stack_params(wuxing):
    # wuxing: (sigma_mem, alpha_ca, alpha_an, i0_ca, i0_an) each (B,1)
...
def stack_infl(influence):
    # influence: (psi, theta_ca, theta_an, phi_ca, phi_an) each (B,1)
    psi, theta_ca, theta_an, phi_ca, phi_an = influence
    return torch.cat([psi, theta_ca, theta_an, phi_ca, phi_an], dim=1)  # (B,5)

def phys_transition_params(pA, mA, eps=1e-9):
    """
    Physics-based state transition (one-step):
        p_{t+1} = f_phys(p_t, m_t)
    pA: (B,5)  mA:(B,5)  both positive / (0,1) where applicable
    """
    ...
    return torch.clamp(p_next, min=eps)
...
# 定义 frequency encoder MLP,将频率数据变为和MLP同样的大小，这样可以对阻抗数据进行频率编码
    ...
    def world_forward_one_step(self, wuxingA, influenceA, env_params_next, electrolyzer_parameters, eps=1e-9):
        """
        Hybrid world-model one-step forward:
        1) latent physical state: pA  (from wuxingA)
        2) latent influence/action: mA (from influenceA)
        3) transition: p_next = f_phys(pA, mA)
        4) observation: V_next_hat = F(p_next, env_next)
        """
        pA = stack_params(wuxingA)          # (B,5)
        mA = stack_infl(influenceA)         # (B,5)
        p_next = phys_transition_params(pA, mA, eps=eps)  # (B,5)

        # env_next: (B,3) -> temperature/flow/current
        temperature = env_params_next[:, 0].unsqueeze(1)
        flow        = env_params_next[:, 1].unsqueeze(1)
        current     = env_params_next[:, 2].unsqueeze(1)

        # 拆回 (B,1) 形式给你的物理电压函数（保持你原函数接口不动）:contentReference[oaicite:2]{index=2}
        sigma_mem, alpha_ca, alpha_an, i_0ca, i_0an = p_next.chunk(5, dim=1)
        psi, theta_ca, theta_an, phi_ca, phi_an = mA.chunk(5, dim=1)

        V_next_hat, V_ocv, V_act, V_diff, V_ohm = calculate_predicted_voltage_plus(
