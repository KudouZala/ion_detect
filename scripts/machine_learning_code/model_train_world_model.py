...
from model_models_world_model import stack_params
...
class Trainer_ThreeSystem_1117(BaseTrainer):
    ...
    def __init__(
        ...
        lambda_forward_p=0.1,
        lambda_forward_v=0.01,
        label_smoothing=0.1,
        ...
        self.lambda_forward_p = lambda_forward_p
        self.lambda_forward_v = lambda_forward_v
    ...
    def compute_loss_pairs(
        ...
        trueVA = batchA[4].to(self.device).squeeze(-1)         # true voltage A        (B, n)#最后一个时刻的电压,n的含义是在T-T+2小时内的电压的采样点个数
        trueVB = batchB[4].to(self.device).squeeze(-1)         # true voltage B        (B, n)#最后一个时刻的电压,n的含义是在T-T+2小时内的电压的采样点个数
        # print("trueVA:",trueVA)
        # print("trueVB:",trueVB)
        ...
        # self.mse_loss_fn_none(predVA, trueVA) 形状为 (B, n),n的含义是在T-T+2小时内的电压的采样点个数，对时间维取均值 → (B,)
        ...
        # ------------------------------------------------------------------
        # 7.5) ★ World-model forward branch (A -> B)
        # ------------------------------------------------------------------
        pA_next_pred, VB_hat_fromA = self.model.world_forward_one_step(
            wuxingA=wuxingA,
            influenceA=influenceA,
            env_params_next=batchB[2].to(self.device),
            electrolyzer_parameters=batchB[5].to(self.device),
            eps=eps
        )

        # (a) 状态转移监督（你原 consistency 的“核心监督”，但现在语义是 forward prediction）
        #     仍然用 dummy_mask 过滤无效 pair
        pA = stack_params(wuxingA)   # (B,5)
        pB = stack_params(wuxingB)  # 你已有 stack_params
        per_sample_p = ((pA_next_pred - pB) ** 2).mean(dim=1)   # (B,)
        valid = (~dummy_mask).float()
        denom = torch.clamp(valid.sum(), min=1.0)
        loss_forward_p = (per_sample_p * valid).sum() / denom

        # (b) 观测预测监督：用 A 推出来的下一状态去预测 B 电压
        #     这里选用 B 的“平均电压”作为标量监督（最稳）
        trueVB_scalar = trueVB.mean(dim=1)  # (B,)
        loss_forward_v = self.mse_loss_fn_mean(VB_hat_fromA, trueVB_scalar)

        # 一个很小的权重就足够把叙事立起来
        lambda_forward_p = self.lambda_forward_p
        lambda_forward_v = self.lambda_forward_v 

        loss_world_forward = lambda_forward_p * loss_forward_p + lambda_forward_v * loss_forward_v
        ...
        log_dict = {
            ...
            "forward_p": loss_forward_p.item(),
            "forward_v": loss_forward_v.item(),
            "world_forward": loss_world_forward.item(),
    ...
    def train_pairs(
        ...
        num_epochs : int
         ...
         eps, use_log_space, lambda_monodec, lambda_polarity :
