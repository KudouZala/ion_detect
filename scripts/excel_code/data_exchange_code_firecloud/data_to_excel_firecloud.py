...
def convert_zhiyun_to_xlsx(base_folder, zhiyun_files, output_folder, remove_first_n_points=0):
    """
    将指定的炙云CSV文件转换为XLSX文件，并保存在output_folder的output_xlsx子文件夹中。

    参数：
    - base_folder: 包含炙云CSV文件的基准文件夹路径。
    - zhiyun_files: 包含炙云CSV文件相对路径的列表。
    - output_folder: XLSX文件的输出文件夹路径。
    - remove_first_n_points: 要移除的前n个数据点。
    """
    # 创建output_xlsx子文件夹路径
    xlsx_output_folder = os.path.join(output_folder, "output_xlsx")
    os.makedirs(xlsx_output_folder, exist_ok=True)
    print(f"XLSX文件将保存在: {xlsx_output_folder}")
    ...
    for file_path_simp in zhiyun_files:
        ...
        xlsx_path = os.path.join(xlsx_output_folder, xlsx_name)

        try:
            df.to_excel(xlsx_path, index=False)
            print(f"已保存 XLSX 文件: {xlsx_path}")
        except Exception as e:
            print(f"无法保存 XLSX 文件 {xlsx_path}: {e}")

def convert_zhiyun_to_xlsx_notitle(base_folder, zhiyun_files, output_folder, remove_first_n_points=0):
    """
    将指定的炙云CSV文件转换为XLSX文件，并保存在output_folder的output_xlsx子文件夹中。
    ...
    for file_path_simp in zhiyun_files:
        ...
        # 生成输出文件名
        base_name = os.path.basename(file_path)
        xlsx_name = os.path.splitext(base_name)[0] + '_no_title.xlsx'
        xlsx_path = os.path.join(xlsx_output_folder, xlsx_name)
