...
def process_csv_files(input_dir, output_dir):
    ...
    if ion_column_files:
        ...
        if output_data:
            ...
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_column_firecloud.xlsx")
    ...
    if ion_files:
        ...
        if output_data:
            final_df = pd.concat(output_data, ignore_index=True)
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_firecloud.xlsx")
            final_df.to_excel(output_file, index=False)
            print(f"已生成: {output_file}")
    ...
    if ion_column_renew_files:
        ...
        if output_data:
            final_df = pd.concat(output_data, ignore_index=True)
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_column_renew_firecloud.xlsx")
            final_df.to_excel(output_file, index=False)
            print(f"已生成: {output_file}")
...
def process_dta_files(input_dirs):
    ...
    output_dir = os.path.join(base_dir, '数据整理')
