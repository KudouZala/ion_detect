...
def process_csv_files(input_dir, output_dir, range):
    ...
    if ion_column_files:
        ...
        if output_data:
            ...
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_column_firecloud_{range}.xlsx")
    ...
    if ion_files:
        ...
        if output_data:
            final_df = pd.concat(output_data, ignore_index=True)
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_firecloud_{range}.xlsx")
            # 假设 final_df 是你的 DataFrame
            # 筛选第一列数据为 0、2、4、6、8、10 的行
            filtered_df = final_df[final_df.iloc[:, 0].isin(range)]
            print(f"生成的filtered_df（前5行）：\n{filtered_df.head()}")
    ...
    if ion_column_renew_files:
        ...
        if output_data:
            final_df = pd.concat(output_data, ignore_index=True)
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_column_renew_H2SO4_firecloud_{range}.xlsx")
            # 假设 final_df 是你的 DataFrame
            # 筛选第一列数据为 0、2、4、6、8、10 的行
            filtered_df = final_df[final_df.iloc[:, 0].isin(range)]
            print(f"生成的filtered_df（前5行）：\n{filtered_df.head()}")
