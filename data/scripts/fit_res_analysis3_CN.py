...
def fit_res_analysis(root_path, ion_density):
    ...
    for ion in ion_list:
        ...
        if ion in mean_diffs_21:
            ...
            plt.plot(labels, vals, marker='x', label=f"{ion} 污染后",
        ...
        if ion in mean_diffs_31:
            ...
            if len(vals)==3:
                labels = ["R_O", "R_LF",  "R_HF"][:len(vals)]
            plt.plot(labels, vals, marker='o', label=f"{ion} 恢复后",
    ...
    plot_path = output_dir / f"resistance_change_plot_2v3_{date_suffix}_{ion_density}_CN.png"
    ...
    excel_filename = f"resistance_change_summary_2v3_{date_suffix}_{ion_density}_CN.xlsx"
