...
def convert_and_plot_3d_surface(folder_path, file_specifications):
    ...
    for idx, (file_name, color, marker) in enumerate(file_specifications):
        ...
        if os.path.isfile(file_path):
            ...
            # 绘制实部的三维曲面
            ax_3d_real.plot(np.log10(df['Freq']), y_value, delta_zreal, label=f"{file_name}", color=color, marker=marker)

            # 绘制虚部的三维曲面
            ax_3d_imag.plot(np.log10(df['Freq']), y_value, delta_zimag, label=f"{file_name}", color=color, marker=marker)
        else:
