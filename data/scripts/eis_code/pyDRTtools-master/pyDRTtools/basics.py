...
def g_i(freq_n, tau_m, epsilon, rbf_type):
    ...
    """ 
       ...
       该函数生成径向基函数（RBF）展开的A_re元素
       Inputs:
            freq_n: frequency频率
            tau_m: log timescale (log(1/freq_m))对数时间尺度（log(1/freq_m)）
            epsilon : shape factor of radial basis functions used for discretization用于离散化的径向基函数的形状因子
            rbf_type: selected RBF type选择的RBF类型
       Outputs:
            Elements of A_re for the RBF expansion,RBF展开的A_re元素
    """
...
def g_ii(freq_n, tau_m, epsilon, rbf_type):
    ...
    """
       ...
       该函数生成径向基函数（RBF）展开的A_im元素
       Inputs:
           freq_n :frequency频率
           tau_m : log timescale (log(1/freq_m))对数时间尺度（log(1/freq_m)）
           epsilon  : shape factor of radial basis functions used for discretization 用于离散化的径向基函数的形状因子
           rbf_type : selected RBF type   选择的RBF类型
       Outputs:
           Elements of A_im for the RBF expansion RBF展开的A_im元素
    """ 
    
    alpha = 2*pi*freq_n*tau_m # 计算中间变量 alpha，用于后续计算。alpha 是频率、时间尺度和常数 2*pi 的乘积。
    ...
    """
    定义一个字典rbf_switch，其中包含不同类型的径向基函数（RBF）。每种类型的RBF都用一个匿名函数（lambda函数）表示，根据输入参数
    epsilon和x计算结果。
    """

    # 根据输入参数 rbf_type 从字典 rbf_switch 中选择相应的RBF函数。如果 rbf_type 不存在于字典中，返回 None。
    rbf = rbf_switch.get(rbf_type)
    # see (33) in [1]#定义积分函数 integrand_g_ii，用于计算积分值。该函数基于参考文献[1]中的方程(33)进行计算。
    integrand_g_ii = lambda x: alpha/(1./exp(x)+(alpha**2)*exp(x))*rbf(x)
    #使用SciPy的 quad 函数计算积分，积分范围从 -50 到 50，绝对误差和相对误差容限分别为 1E-9。
