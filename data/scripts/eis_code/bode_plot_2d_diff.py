...
def convert_and_plot_3d_surface_and_2d_diff(folder_path, file_specifications, frequencies):
    ...
    for idx, (file_name, color, marker) in enumerate(file_specifications):
        ...
        if os.path.isfile(file_path):
            ...
            if previous_real is not None:
                delta_zreal = df['Zreal'] - previous_real
                delta_zimag = df['Zimag'] - previous_imag
            else:
                delta_zreal = np.zeros_like(df['Zreal'])
                delta_zimag = np.zeros_like(df['Zimag'])

            all_delta_zreal.append(delta_zreal)
            all_delta_zimag.append(delta_zimag)

            previous_real = df['Zreal']
            previous_imag = df['Zimag']

            # 绘制实部的三维曲面
            ax_3d_real.plot(np.log10(df['Freq']), y_value, delta_zreal, label=f"{file_name}", color=color,
                            marker=marker)

            # 绘制虚部的三维曲面
            ax_3d_imag.plot(np.log10(df['Freq']), y_value, delta_zimag, label=f"{file_name}", color=color,
                            marker=marker)
        else:
    ...
    all_delta_zreal = pd.DataFrame(all_delta_zreal).values
    all_freq_imag = pd.DataFrame(all_freq_imag).values
    all_delta_zimag = pd.DataFrame(all_delta_zimag).values

    # 绘制实部曲面
    ax_3d_real.plot_surface(all_freq_real, y_values, all_delta_zreal, cmap='viridis', alpha=0.6)

    ax_3d_real.set_xlabel('Log(Frequency) (Hz)')
    ax_3d_real.set_ylabel('File Index')
    ax_3d_real.set_zlabel('Delta Zreal (Ohm)')
    ax_3d_real.set_title('3D Bode Surface Plot - Delta Real Part')

    # 绘制虚部曲面
    ax_3d_imag.plot_surface(all_freq_imag, y_values, all_delta_zimag, cmap='plasma', alpha=0.6)

    ax_3d_imag.set_xlabel('Log(Frequency) (Hz)')
    ax_3d_imag.set_ylabel('File Index')
    ax_3d_imag.set_zlabel('Delta Zimag (Ohm)')
    ax_3d_imag.set_title('3D Bode Surface Plot - Delta Imaginary Part')

    # 保存图像
    real_plot_path = os.path.join(output_folder_plot, "bode_real_plot_3d_delta.png")
    imag_plot_path = os.path.join(output_folder_plot, "bode_imag_plot_3d_delta.png")

    fig_3d_real.savefig(real_plot_path)
    fig_3d_imag.savefig(imag_plot_path)

    # 绘制并保存指定频率下的2D图
    plot_2d_diff_for_frequencies(folder_path, frequencies, file_specifications, all_freq_real, all_delta_zreal,
                                 all_delta_zimag)

    plt.show()
def main():
    base_folder = r"C:\Users\asus\Desktop\ROG桌面其他文件\电解水\20240820铜离子污染测试\20240820Cuion\EISGALV_60℃_150ml_1A"

    # 指定每个文件的文件名、颜色和标记符号
    file_specifications = [
        # ("cm2_20240819_ion_column_1.DTA", 'red', 's'),  # 红色圆形
        ("cm2_20240819_ion_column_4.DTA", 'orange', 's'),  # 红色方形
        ("cm2_20240819_ion_column_8.DTA", 'yellow', 's'),  # 红色方形
        ("cm2_20240819_ion_column_12.DTA", 'green', 's'),  # 红色方形
        ("cm2_20240819_ion_column_16.DTA", 'blue', 's'),  # 红色方形
        ("cm2_20240819_ion_column_20.DTA", 'cyan', 's'),  # 红色方形
        ("cm2_20240819_ion_column_24.DTA", 'purple', 's'),  # 红色方形
        ("cm2_20240819_ion_column_28.DTA", 'brown', 's'),  # 红色方形
        # ("cm2_20240819_ion_column_32.DTA", 'black', 's'),  # 红色方形

        # ("cm2_20240820_no_ion_column_1.DTA", 'red', 's'),  # 红色圆形
        # ("cm2_20240820_no_ion_column_4.DTA", 'orange', 's'),  # 红色方形
        # ("cm2_20240820_no_ion_column_8.DTA", 'yellow', 's'),  # 红色方形
        # ("cm2_20240820_no_ion_column_12.DTA", 'green', 's'),  # 红色方形
        # ("cm2_20240820_no_ion_column_16.DTA", 'blue', 's'),  # 红色方形
        # ("cm2_20240820_no_ion_column_20.DTA", 'cyan', 's'),  # 红色方形
        # ("cm2_20240820_no_ion_column_24.DTA", 'purple', 's'),  # 红色方形
        # ("cm2_20240820_no_ion_column_28.DTA", 'brown', 's'),  # 红色方形
        # ("cm2_20240820_no_ion_column_32.DTA", 'black', 's'),  # 红色方形

        # ("cm2_20240820_ion_1.DTA", 'red', '+'),  # 红色圆形
        # ("cm2_20240820_ion_4.DTA", 'orange', '+'),  # 红色方形
        # ("cm2_20240820_ion_8.DTA", 'yellow', '+'),  # 红色方形
        # ("cm2_20240820_ion_12.DTA", 'green', '+'),  # 红色方形
        # ("cm2_20240820_ion_16.DTA", 'blue', '+'),  # 红色方形
        # ("cm2_20240820_ion_20.DTA", 'cyan', '+'),  # 红色方形
        # ("cm2_20240820_ion_24.DTA", 'purple', '+'),  # 红色方形
        # ("cm2_20240820_ion_28.DTA", 'brown', '+'),  # 红色方形
        # ("cm2_20240820_ion_32.DTA", 'black', '+'),  # 红色方形
