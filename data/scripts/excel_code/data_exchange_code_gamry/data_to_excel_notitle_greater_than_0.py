...
def convert_to_csv_and_xlsx_no_title_greater_than_0(folder_path, file_specifications):
    ...
    for file_name in file_specifications:
        ...
        if os.path.isfile(file_path):
            ...
            # 保存为 CSV 文件
            csv_file_path = os.path.join(output_folder_csv, f"{base_name}_no_title_greater_than_0.csv")
            df.to_csv(csv_file_path, index=False,header = False)
            print(f"已保存 CSV 文件: {csv_file_path}")

            # 保存为 Excel 文件
            xlsx_file_path = os.path.join(output_folder_xlsx, f"{base_name}_no_title_greater_than_0.xlsx")
...
def main():
    base_folder = r"C:\Users\asus\Desktop\ROG桌面其他文件\电解水\校内测试\20240823_10ppm钙离子污染和恢复测试\新版电解槽_gamry\EISGALV_60℃_150ml_1A"
