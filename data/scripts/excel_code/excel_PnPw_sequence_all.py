...
import os
import re
def sort_fit_res(folder_path,ion_type):
    def find_target_files_and_titles(root_dir):
        ...
        for sub_dir_name in os.listdir(root_dir):
            ...
            if os.path.isdir(sub_dir_path) and ion_type in sub_dir_name:
                # 提取信息：日期、ppm、离子种类
                match = re.search(r"(\d{8})_([\d.]+ppm)(" + "|".join(ion_map.keys()) + ")", sub_dir_name)
    ...
    # ✅ 拼接目标根目录
    root_folder = os.path.join(parent_3, "eis_fit_results", folder_path)
...
folder_path = "20250723"

for ion_type in ion_types:
    sort_fit_res(folder_path=folder_path,ion_type=ion_type)