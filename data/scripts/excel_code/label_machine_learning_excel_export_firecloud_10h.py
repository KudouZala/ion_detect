...
def process_csv_files(input_dir, output_dir):
    ...
    if ion_column_files:
        ...
        if output_data:
            ...
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_column_firecloud_10h.xlsx")
            # 假设 final_df 是你的 DataFrame
            # 筛选第一列数据为 0、2、4、6、8、10 的行
            filtered_df = final_df[final_df.iloc[:, 0].isin([0, 2, 4, 6, 8, 10])]
    ...
    if ion_files:
        ...
        if output_data:
            final_df = pd.concat(output_data, ignore_index=True)
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_firecloud_10h.xlsx")
            # 假设 final_df 是你的 DataFrame
            # 筛选第一列数据为 0、2、4、6、8、10 的行
            filtered_df = final_df[final_df.iloc[:, 0].isin([0, 2, 4, 6, 8, 10])]
            print(f"生成的filtered_df（前5行）：\n{filtered_df.head()}")
    ...
    if ion_column_renew_files:
        ...
        if output_data:
            final_df = pd.concat(output_data, ignore_index=True)
            output_file = os.path.join(output_dir, f"{os.path.basename(parent_dir_name)}_ion_column_renew_firecloud_10h.xlsx")
            # 假设 final_df 是你的 DataFrame
            # 筛选第一列数据为 0、2、4、6、8、10 的行
            filtered_df = final_df[final_df.iloc[:, 0].isin([0, 2, 4, 6, 8, 10])]
            print(f"生成的filtered_df（前5行）：\n{filtered_df.head()}")
...
def process_dta_files(input_dirs):
    ...
    for input_dir in input_dirs:
        ...
        # 处理该文件夹下的 CSV 文件
        process_csv_files(input_folder, output_dir)
