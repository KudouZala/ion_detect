...
def calculate_fc(Q, R, alpha):
    try:
        ...
        if base <= 0 or alpha <= 0:
            return float('nan')  # 或者 raise ValueError("Invalid input values.")
        if base > 1e100 or 1/alpha > 100:  # 限制数量级
            return float('inf')  # 表示爆炸
        fc = base ** (1 / alpha) / (2 * math.pi)
        return fc
    except OverflowError:
        print(f"Overflow error for Q={Q}, R={R}, alpha={alpha}")
        return float('inf')  # 或 float('nan')，或 return None
...
def process_multiple_files(file_paths,legend_dict,custom_title):
    # 获取当前py文件的所在路径
    # base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../eis_fit_results/'))
    num=0
