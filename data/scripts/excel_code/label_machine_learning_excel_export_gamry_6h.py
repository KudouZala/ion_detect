...
def save_data_to_excel(input_dir, dta_files, output_dir):
    ...
    if '_ion_column_renew_H2SO4_' in dta_file:
        output_file = os.path.join(output_dir, f"{input_dir.split(os.sep)[-3]}_ion_column_renew_H2SO4_gamry_6h.xlsx")
    elif '_ion_column_' in dta_file:
        output_file = os.path.join(output_dir, f"{input_dir.split(os.sep)[-3]}_ion_column_gamry_6h.xlsx")
    elif '_ion_' in dta_file:
        output_file = os.path.join(output_dir, f"{input_dir.split(os.sep)[-3]}_ion_gamry_6h.xlsx")
    

    # 假设 final_df 是你的 DataFrame
    # 筛选第一列数据为 0、2、4、6、8、10 的行
    filtered_df = final_df[
    final_df.iloc[:, 0].isin([0, 2, 4, 6]) & (final_df.iloc[:, 1] < 22000)
    ]
