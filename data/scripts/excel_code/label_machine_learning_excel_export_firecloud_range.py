...
def detect_cell_version(path_str: str) -> str:
    ...
    if '旧版电解槽' in path_str:
        return '旧版电解槽'
    return '未知电解槽'
def extract_data_from_csv(csv_file, time_value, ppm_value, label_value,mean_voltage,temperature=60,flow=150,current=1):
    """
...
def process_csv_files(input_dir, output_dir, range):
    ...
    voltage_ion_column_files=glob.glob(os.path.join(input_dir, '*_ion_column', '*(CC)*.csv'))
    voltage_ion_files=glob.glob(os.path.join(input_dir, '*_ion', '*(CC)*.csv'))
    voltage_ion_column_renew_files=glob.glob(os.path.join(input_dir, '*_ion_column_renew', '*(CC)*.csv'))
    ...
    print("ion_column_renew_files:",ion_column_renew_files)

    voltage_ion_column_files.sort(key=custom_sort)
    voltage_ion_files.sort(key=custom_sort)
    voltage_ion_column_renew_files.sort(key=custom_sort)
    ...
    if ion_files:
        ...
        for i, csv_file in enumerate(ion_files):
            voltage_file = voltage_ion_files[i]
            mean_voltage = pd.read_csv(voltage_file)['Voltage/V'].mean()
            time_value = 2 * i
            df = extract_data_from_csv(csv_file, time_value, ppm_value, label_value,mean_voltage,temperature=temp_value,flow=flow_value)
            if df is not None:
        ...
        if output_data:
            ...
            output_file = os.path.join(
                            output_dir,
                            f"{os.path.basename(parent_dir_name)}_{cell_version}_ion_firecloud_{range}.xlsx"
           ...
           # 检查my_range中的全部数字是否都在filtered_df的第一列中
            if all(num in filtered_df.iloc[:, 0].values for num in range):
                filtered2_df = filtered_df
                filtered2_df.iloc[:, 0]=filtered2_df.iloc[:, 0]-range[0]                      
    ...
    if ion_column_files:
        ...
        for i, csv_file in enumerate(ion_column_files):
            voltage_file = voltage_ion_column_files[i]
            mean_voltage = pd.read_csv(voltage_file)['Voltage/V'].mean()
            ppm_value=0
            time_value = 2 * i
            df = extract_data_from_csv(csv_file, time_value, ppm_value, "no_ion",mean_voltage,temperature=temp_value,flow=flow_value)
            if df is not None:
                output_data.append(df)
        ...
        if output_data:
            ...
            output_file = os.path.join(
                output_dir,
                f"{os.path.basename(parent_dir_name)}_{cell_version}_ion_column_firecloud_{range}.xlsx"
            )            # 假设 final_df 是你的 DataFrame
            # 筛选第一列数据为 0、2、4、6、8、10 的行
            filtered_df = final_df[final_df.iloc[:, 0].isin(range)]
            print(f"生成的filtered_df（前5行）：\n{filtered_df.head()}")
    ...
    if ion_column_renew_files:
        ...
        for i, csv_file in enumerate(ion_column_renew_files):
            voltage_file = voltage_ion_column_renew_files[i]
            mean_voltage = pd.read_csv(voltage_file)['Voltage/V'].mean()
            time_value = 2 * i
            df = extract_data_from_csv(csv_file, time_value, ppm_value, "no_ion",mean_voltage,temperature=temp_value,flow=flow_value)
            if df is not None:
        ...
        if output_data:
            ...
            output_file = os.path.join(
                            output_dir,
                            f"{os.path.basename(parent_dir_name)}_{cell_version}_ion_column_renew_H2SO4_firecloud_{range}.xlsx"
                        )            # 假设 final_df 是你的 DataFrame
            # 筛选第一列数据为 0、2、4、6、8、10 的行
            filtered_df = final_df[final_df.iloc[:, 0].isin(range)]
            print(f"生成的filtered_df（前5行）：\n{filtered_df.head()}")
            # 可以在这里添加更多操作，如果想进一步检查缺失值等
            # 检查my_range中的全部数字是否都在filtered_df的第一列中
            if all(num in filtered_df.iloc[:, 0].values for num in range):
                filtered2_df = filtered_df
                filtered2_df.iloc[:, 0]=filtered2_df.iloc[:, 0]-range[0]                      
...
def process_dta_files(input_dirs, range_vals):
    """
    处理多个文件夹中的 DTA/CSV 文件，提取数据并保存为 Excel 文件。
    跨平台路径兼容：自动识别 Windows/Ubuntu 分隔符。
    """
    # 基目录：上三级目录 + “校内测试”
    ...
    for input_dir in input_dirs:
        # 统一切分：把 \ 或 / 都视为分隔符，再按当前系统规则拼接
        parts = re.split(r'[\\/]+', input_dir.strip().strip("\\/"))
        input_folder = os.path.join(base_dir, *parts)
        ...
        # 处理该文件夹下的 CSV 文件
        process_csv_files(input_folder, output_dir, range_vals)
